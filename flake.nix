...

{
  ...
  inputs = {
    ...
    diskoManager.url = "github:nix-community/disko/latest";
    diskoManager.inputs.nixpkgs.follows = "nixosStable";
  };
  ...
  outputs = {
    ...
    ## 3. Argumentos de pacotes do nixpkgs
    commonPkgs = {
      stablePkgs = nixosStable.legacyPackages.x86_64-linux; # <--- Pacotes estáveis do nixpkgs
      unstablePkgs = nixosUnstable.legacyPackages.x86_64-linux; # <--- Pacotes instáveis do nixpkgs
    };
    ## 4. Perfis reutilizáveis gerados AUTOMATICAMENTE a partir da DASHBOARD
    ...
    ## 5. Função construtora de hosts: Monta a configuração de um host
    buildHostConfig = { configName, machineProfile, hostName, userName, flags ? {}, extraModules ? [], extraSpecialArgs ? {} }: {
      ...
      specialArgs = criticalArgs // commonPkgs // ( machineProfile.specialArgs or {} ) // flags // extraSpecialArgs // { inherit hostName userName; };
    };
    ...
    hostsByArch = {
      "x86_64-linux" = {
        ...
        "vm_0" = { ### <--- Nome de busca (apelido que usa para o nix (ex:nixos-rebuild switch --flake .#apelido))
          machineProfile = machineProfiles.vmware;
          ...
          flags = {
            ...
            isVM = true; ### <--- É uma VM?
            usesDisko = false; ### <--- Usa particionamento automático com Disko?
          };
        };
      };
    };
        ...
        nixosStable.lib.mapAttrs ( system: hosts: 
          ...
          nixosStable.lib.mapAttrs ( configName: hostConfig: 
            ...
            nixosStable.lib.nixosSystem (
              ...
              ( buildHostConfig ( hostConfig // { inherit configName; })) // { inherit system; }
            )
          ) hosts
        ) hostsByArch
      )
